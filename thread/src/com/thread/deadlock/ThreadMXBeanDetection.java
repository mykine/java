...
//        System.out.println("r2.flag="+ r2.getFlag());
        ...
        //使用ThreadMXBean监测死锁进程列表
        ThreadMXBean threadMXBean = ManagementFactory.getThreadMXBean();
        long[] deadlockedThreadIds = threadMXBean.findDeadlockedThreads();
        if( deadlockedThreadIds!=null && deadlockedThreadIds.length>0){
            System.out.println("监测到死锁线程:");
            for (int i = 0; i < deadlockedThreadIds.length; i++) {
                ThreadInfo threadInfo = threadMXBean.getThreadInfo(deadlockedThreadIds[i]);
                System.out.println(threadInfo.getThreadName()+":"+threadInfo.getLockName());
            }
        }
    }
